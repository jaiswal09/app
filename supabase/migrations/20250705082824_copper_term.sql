/*
  # Fix Billing Function JSON Parameter Issue
  
  This migration fixes the JSON parameter handling in the billing functions
  to resolve the "cannot extract elements from a scalar" error.
*/

-- Drop and recreate the billing function with proper JSON handling
DROP FUNCTION IF EXISTS create_bill_with_inventory_update(text, text, text, text, date, date, decimal, decimal, text, text, jsonb);

-- Create the corrected function with proper JSON parameter handling
CREATE OR REPLACE FUNCTION create_bill_with_inventory_update(
  ...
  p_items text DEFAULT '[]'  -- Changed from jsonb to text to handle the parameter properly
)
RETURNS uuid AS $$
DECLARE
  bill_id uuid;
...
DROP FUNCTION IF EXISTS update_bill_with_inventory_update(uuid, text, text, text, text, date, date, decimal, decimal, text, text, jsonb);
...
CREATE OR REPLACE FUNCTION update_bill_with_inventory_update(
  ...
  p_notes text DEFAULT NULL,
  p_status text DEFAULT 'draft',
  p_items text DEFAULT '[]'  -- Changed from jsonb to text
)
RETURNS void AS $$
DECLARE
